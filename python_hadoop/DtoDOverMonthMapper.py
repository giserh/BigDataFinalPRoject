    ...
    for values in parseInput():
        if values[10] and values[11] and values[12] and values[13]:
            ...
            if pickup_neighborhood!=-1 and drop_neighborhood !=-1:
                d_to_d = Month + '|' + str(pickup_neighborhood) + '|' + str(drop_neighborhood) 
                agg[d_to_d] = agg.get(d_to_d, 0) + 1

    for item in agg.iteritems():
        (month, pickup_index,drop_index) = item[0].split('|')
        pickup_name = neighborhoods[int(pickup_index)][0]
        drop_name = neighborhoods[int(drop_index)][0]
        d_to_d_key = month + '|' + pickup_name + '|' + drop_name
        print '%s\t%s' % (d_to_d_key, item[1])
